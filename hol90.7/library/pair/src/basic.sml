...
(* ---------------------------------------------------------------------*)
(*$Id: basic.ml,v 2.1 1992/11/12 09:48:50 jg Rel $*)
...
signature Pair_basic_sig =
    sig
	val MK_PAIR : thm * thm -> thm
	val PABS : term -> thm -> thm
	val PABS_CONV : (term -> thm) -> term -> thm
	val PSUB_CONV : (term -> thm) -> term -> thm
	val CURRY_CONV : term -> thm
	val UNCURRY_CONV : term -> thm
	val PBETA_CONV : term -> thm
	val PBETA_RULE : thm -> thm
	val PBETA_TAC : tactic
	val RIGHT_PBETA : thm -> thm
	val LIST_PBETA_CONV : term -> thm
	val RIGHT_LIST_PBETA : thm -> thm
	val LEFT_PBETA : thm -> thm
	val LEFT_LIST_PBETA : thm -> thm
	val UNPBETA_CONV : term -> term -> thm
	val PETA_CONV : term -> thm
	val PALPHA_CONV : term -> conv
	val GEN_PALPHA_CONV : term -> term -> thm
	val PALPHA : term -> term -> thm
	val paconv : term -> term -> bool
	val PAIR_CONV : (term -> thm) -> term -> thm
    end;
    
structure Pair_basic:Pair_basic_sig =
struct
    open Rsyntax;	
    open Pair_syn;
...
fun PABS_CONV conv tm =
    let val {Bvar,Body} = (dest_pabs tm handle _ => failwith "PABS_CONV") 
	val bodyth = conv Body
    in
	(PABS Bvar bodyth handle _ => failwith "PABS_CONV")
    end;
...
handle _ => failwith "UNCURRY_CONV" ;
...
fun RIGHT_PBETA th =
    TRANS th (PBETA_CONV (rhs (concl th))) handle _ => failwith "RIGHT_PBETA";
...
fun UNPBETA_CONV v tm =
    ...
    handle _ => failwith "UNPBETA_CONV";
...
fun GEN_PALPHA_CONV p tm = 
	...
	failwith ""
	handle _ => failwith "GEN_PALPHA_CONV";
(* ------------------------------------------------------------------------- *)
