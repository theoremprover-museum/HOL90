...
functor ConsThmsFunc(MutRecDef:MutRecDefSig):ConsThms_sig =
struct

(* use records *)
open Rsyntax;

val rec_thm = MutRecDef.New_Ty_Existence_Thm
val induct_thm = MutRecDef.New_Ty_Induct_Thm
...
val dist_aux_ftns_thm = define_mutual_functions
...
(* Now I have to define some functions that, given a constructor CON applied
   ...
   extraction function willbe called CON_arg.) *)
...
fun define_single_ftn con arg =
	...
	val ftn_thm = define_mutual_functions
...
fold (fn (listlist,name_thm_list)=> 
       fold (fn (name_thm_const_list, name_thm_l) => 
	      fold (fn ((n_t,c),n_t_list) => n_t::n_t_list)
	           name_thm_const_list
		   name_thm_l)
            listlist
	    name_thm_list)
