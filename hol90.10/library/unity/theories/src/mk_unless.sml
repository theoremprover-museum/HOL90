...
if (current_theory() <> "state_logic") then
    load_theory"state_logic"
else
    ();

delete_theory (path^"unless");
new_theory"unless";

(*-------------------------------------------------------------------------*)
(* Loading definitions from parent theories			           *)
(*-------------------------------------------------------------------------*)
...
val AND_AND_lemma        = theorem"state_logic""AND_AND_lemma";
...
val IMPLY_WEAK_lemma1    = theorem"state_logic""IMPLY_WEAK_lemma1";
val IMPLY_WEAK_lemma2    = theorem"state_logic""IMPLY_WEAK_lemma2";
val IMPLY_WEAK_lemma3    = theorem"state_logic""IMPLY_WEAK_lemma3";
val IMPLY_WEAK_lemma4    = theorem"state_logic""IMPLY_WEAK_lemma4";
val NOT_AND_OR_NOT_lemma = theorem"state_logic""NOT_AND_OR_NOT_lemma";
...
val OR_NOT_AND_lemma     = theorem"state_logic""OR_NOT_AND_lemma";
...
(* The definition of UNLESS is based on the definition:
  ...
  where p /\'a q are state dependent first \/'ader logic predicates /\'a all
...
(*
  ...
  a separate HOARE tripple relation between pre- /\'a post predicates to be 
  satisfied for state transitions. The pre- /\'a post predicates of the
  UNLESS_STMT relation must be satisfiable for all states possible in the
  finite state space of the program.
*)
val UNLESS_STMT = new_infix_definition
  ("UNLESS_STMT",
   (--`UNLESS_STMT (p:'a->bool) q st =
      \s:'a. (p s /\ ~q s) ==> (p (st s) \/ q (st s))`--),
   425);
...
val UNLESS = new_recursive_definition
    {name = "UNLESS",
     def  = --`(UNLESS p q  []                   = T) /\
               (UNLESS p q (CONS (st:'a->'a) Pr) =
...
*)
val STABLE = new_infix_definition
  ("STABLE", --`STABLE (p:'a->bool) Pr = (p UNLESS False) Pr`--, 425);
...
*)
val INVARIANT = new_infix_definition
  ("INVARIANT",
   --`INVARIANT p (p0, Pr) = (!s:'a. p0 s ==> p s) /\ (p STABLE Pr)`--,
   425);
...
val IMP_IMP_CONJIMP_lemma = TAC_PROOF
  (([],
...
val IMP_IMP_DISJIMP_lemma = TAC_PROOF
  (([],
   --`!p q ps qs p' q' p's q's.
    (p /\ ~q ==> ps \/ qs) /\ (p' /\ ~q' ==> p's \/ q's) ==>
      ((p \/ p') /\ (p \/ ~q') /\ (p' \/ ~q) /\ (~q \/ ~q') ==>
   ...
   DISCH_TAC THEN
   REWRITE_TAC [
     SPECL [(--`ps:bool`--),(--`p's:bool`--),
	    (--`~ps /\ q's \/ ~p's /\ qs \/ qs /\ q's`--)]
 ...
 *)
val UNLESS_STMT_thm0 = prove_thm
  ("UNLESS_STMT_thm0",
 ...
 *)
val UNLESS_STMT_thm1 = prove_thm
  ("UNLESS_STMT_thm1",
   --`!(p:'a->bool) q r st.
   ...
   STRIP_ASSUME_TAC (SPEC_ALL (ASSUME
     (--`!s. (\s. p s /\ ~q s ==> p ((st:'a->'a) s) \/ q (st s))s`--))) THEN
   UNDISCH_TAC
     (--`(\s. p s /\ ~q s ==> p ((st:'a->'a) s) \/ q (st s))s`--) THEN
   PURE_REWRITE_TAC [BETA_CONV
     (--`(\s. (p s /\ ~q s) ==> (P((st:'a->'a)s) \/  q (st s)))s`--)] THEN
   REPEAT STRIP_TAC THEN
   STRIP_ASSUME_TAC (CONTRAPOS (SPEC (--`s:'a`--)
      (ASSUME (--`!s:'a. (q s) ==> (r s)`--)))) THEN
   RES_TAC THENL
     [ASM_REWRITE_TAC []
     ,RES_TAC THEN (ASM_REWRITE_TAC [])
     ]);
...
*)
val UNLESS_STMT_thm2 = prove_thm
  ("UNLESS_STMT_thm2",
   --`!p q p' q' (st:'a->'a).
...
*)
val UNLESS_STMT_thm3 = prove_thm
  ("UNLESS_STMT_thm3",
   --`!p q p' q' (st:'a->'a).
     ((!s. (p UNLESS_STMT q) st s) /\ (!s. (p' UNLESS_STMT q') st s)) ==>
      (!s. ((p /\* p') UNLESS_STMT
...
(*
  Disjunction Theorem:
     ...
     (p \/ p') unless_stmt (~p /\ q') \/ (~p' /\ q) \/ (q /\ q') in stmt
*)
val UNLESS_STMT_thm4 = prove_thm 
  ("UNLESS_STMT_thm4",
   --`!p q p' q' (st:'a->'a).
     ((!s. (p UNLESS_STMT q) st s) /\ (!s. (p' UNLESS_STMT q') st s)) ==>
      (!s. ((p \/* p') UNLESS_STMT
    ...
    ASSUME_TAC (SPEC_ALL (CONJUNCT1 (ASSUME
      (--`(!s. p s /\ ~q s ==> p ((st:'a->'a) s) \/ q (st s)) /\
          (!s. p' s /\ ~q' s ==> p'((st:'a->'a) s) \/ q'(st s))`--)))) THEN
    ASSUME_TAC (SPEC_ALL (CONJUNCT2 (ASSUME
      (--`(!s. p s /\ ~q s ==> p ((st:'a->'a) s) \/ q (st s)) /\
          (!s. p' s /\ ~q' s ==> p'((st:'a->'a) s) \/ q'(st s))`--)))) THEN
    ASSUME_TAC (UNDISCH_ALL (SPECL
  [(--`(p s /\ ~q s ==> p ((st:'a->'a) s) \/ q (st s))`--),
   (--`(p' s /\ ~q' s ==> p'((st:'a->'a) s) \/ q'(st s))`--)]
   AND_INTRO_THM)) THEN
    DISCH_TAC THEN
    ASSUME_TAC (UNDISCH_ALL
   (SPECL [(--`(p:'a->bool) s`--),(--`(q:'a->bool) s`--),
           (--`(p:'a->bool) ((st:'a->'a)s)`--),
           (--`(q:'a->bool) ((st:'a->'a)s)`--),
           (--`(p':'a->bool) s`--),(--`(q':'a->bool) s`--),
           (--`(p':'a->bool) ((st:'a->'a)s)`--),
           (--`(q':'a->bool) ((st:'a->'a)s)`--)]
               IMP_IMP_DISJIMP_lemma)) THEN
    ASM_REWRITE_TAC []);

val UNLESS_STMT_thm5_lemma1 = TAC_PROOF
  (([],
   --`!p q r. (p ==> q) ==> (p \/ r ==> q \/ r)`--),
   REPEAT STRIP_TAC THENL
     [RES_TAC THEN ASM_REWRITE_TAC []
...
val UNLESS_STMT_thm5_lemma2 = TAC_PROOF
  (([],
...
val UNLESS_STMT_thm5 = prove_thm 
  ("UNLESS_STMT_thm5",
   --`!(P:num->('a->bool)) q st.
   ...
   LIST_INDUCT_TAC THENL
     ...
     , GEN_TAC THEN
       ...
       CONJ_TAC THENL
         [ GEN_TAC THEN
           PURE_REWRITE_TAC [BETA_CONV
             (--`(\s. (p:'a->bool) s /\ ~p s ==> p (h s) \/ p (h s))s`--)] THEN
           REPEAT STRIP_TAC THEN
           RES_TAC
   ...
   --`!(p:'a->bool) q r Pr.
    ...
    LIST_INDUCT_TAC THENL
      ...
      , X_GEN_TAC (--`st:'a->'a`--) THEN
        ...
        REPEAT STRIP_TAC THENL
          [ ASSUME_TAC (UNDISCH_ALL
              (SPECL [(--`(!s:'a. (p UNLESS_STMT q)st s)`--),
                      (--`(!s:'a. q s ==> r s)`--)] AND_INTRO_THM)) THEN
...
val UNLESS_cor3a = TAC_PROOF
  (([],
   (--`!(p:'a->bool) q r Pr.
          (p UNLESS (q \/* r)) Pr ==>
          ((p /\* (~* q)) UNLESS (q \/* r)) Pr`--)),
   REPEAT GEN_TAC THEN
   ASSUME_TAC (SPECL [(--`~* (q:'a->bool)`--),
                      (--`Pr:('a->'a)list`--)] UNLESS_thm2) THEN
   UNDISCH_TAC (--`((~* (q:'a->bool)) UNLESS (~*(~* q)))Pr`--) THEN
   REWRITE_TAC [NOT_NOT_lemma] THEN
   REPEAT STRIP_TAC THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
     [(--`((p:'a->bool) UNLESS (q \/* r))Pr`--),
      (--`((~* (q:'a->bool)) UNLESS q)Pr`--)]
      AND_INTRO_THM)) THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
     [(--`p:'a->bool`--),(--`((q:'a->bool) \/* r)`--),
      (--`(~* (q:'a->bool))`--),(--`q:'a->bool`--),(--`Pr:('a->'a)list`--)]
      UNLESS_thm6)) THEN
  ...
  (([],
   ...
   ASSUME_TAC (UNDISCH_ALL (SPECL
     [(--`(p:'a->bool) /\* q`--),(--`(p:'a->bool) /\* q`--),
      ...
      UNLESS_thm3)) THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
     [(--`(((p:'a->bool) /\* (~* q)) UNLESS (q \/* r))Pr`--),
      (--`(((p:'a->bool) /\* q) UNLESS q)Pr`--)]
      AND_INTRO_THM)) THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
     [(--`((p:'a->bool) /\* (~* q))`--),(--`((q:'a->bool) \/* r)`--),
      (--`((p:'a->bool) /\* q)`--),(--`q:'a->bool`--),(--`Pr:('a->'a)list`--)]
      UNLESS_thm7)) THEN
   UNDISCH_TAC
   (--`((((p:'a->bool) /\* (~* q)) \/* (p /\* q)) UNLESS
        ((q \/* r) \/* q))Pr`--) THEN
...
val UNLESS_cor4 = prove_thm
  ("UNLESS_cor4",
   (--`!(p:'a->bool) q r Pr.
               ((p \/* q) UNLESS r) Pr ==> (p UNLESS (q \/* r)) Pr`--),
   REPEAT STRIP_TAC THEN
   ASSUME_TAC (SPEC_ALL ((SPEC (--`~* (q:'a->bool)`--) UNLESS_thm2))) THEN
   UNDISCH_TAC (--`((~* (q:'a->bool)) UNLESS (~*(~* q)))Pr`--) THEN
   REWRITE_TAC [NOT_NOT_lemma] THEN
   STRIP_TAC THEN
   ASSUME_TAC (UNDISCH_ALL (SPEC_ALL (SPECL
   ...
   ASSUME_TAC (UNDISCH_ALL (SPEC_ALL (SPECL
         [(--`((p:'a->bool) \/* q)`--),(--`r:'a->bool`--),
          (--`(~* (q:'a->bool))`--),(--`q:'a->bool`--)]
          UNLESS_thm6))) THEN
   UNDISCH_TAC (--`((((p:'a->bool) \/* q) /\* (~* q)) UNLESS
                    (r \/* q))Pr`--) THEN
  ...
  ("UNLESS_cor5",
   ...
   ASSUME_TAC (UNDISCH_ALL (SPEC_ALL (SPECL
         [(--`p:'a->bool`--),(--`p:'a->bool`--),
          (--`p:'a->bool`--),(--`(~* (p:'a->bool))`--)]
          UNLESS_thm6))) THEN
   UNDISCH_TAC (--`(((p:'a->bool) /\* p) UNLESS (p \/* (~* p)))Pr`--) THEN
   REWRITE_TAC [AND_AND_lemma,P_OR_NOT_P_lemma]);

val UNLESS_cor6 = prove_thm
  ("UNLESS_cor6",
   (--`!(p:'a->bool) Pr. (True UNLESS p) Pr`--),
   REPEAT GEN_TAC THEN
   ASSUME_TAC (SPEC_ALL UNLESS_thm1) THEN
   ASSUME_TAC (SPEC_ALL (SPEC (--`(~* (p:'a->bool))`--) UNLESS_thm2)) THEN
   UNDISCH_TAC (--`((~* (p:'a->bool)) UNLESS (~*(~* p)))Pr`--) THEN
   REWRITE_TAC [NOT_NOT_lemma] THEN
   DISCH_TAC THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
         [(--`((~* (p:'a->bool)) UNLESS p)Pr`--),
...
val HeJiFeng_lemma_f = MK_ABS (UNDISCH_ALL (SPEC_ALL HeJiFeng_lemma));

val UNLESS_cor8 = prove_thm
  ("UNLESS_cor8",
   (--`!(p:'a->bool) q p' Pr.
       (!s. p s /\ ~q s) ==> (!s. p' s) ==> (!s. p s \/ q s)
            ==> (((p  /\* (~* q)) UNLESS q) Pr =
...
val UNLESS_cor11 = prove_thm
  ("UNLESS_cor11",
   (--`!(p:'a->bool) Pr. (!s. (~* p)s) ==> p STABLE Pr`--),
   GEN_TAC THEN
   REWRITE_TAC [STABLE] THEN
   LIST_INDUCT_TAC THENL
     [
     ...
     ,
      ...
      DISCH_TAC THEN
      RES_TAC THEN
      ASM_REWRITE_TAC [False] THEN
      UNDISCH_TAC (--`!s:'a. ~* p s`--) THEN
      REWRITE_TAC [~*] THEN
      CONV_TAC (DEPTH_CONV BETA_CONV) THEN
      REPEAT STRIP_TAC THEN
      RES_TAC
...
val UNLESS_cor14 = prove_thm
  ("UNLESS_cor14",
   (--`!(p:'a->bool) q Pr. (p UNLESS (~* q)) Pr /\ q STABLE Pr ==>
                               (p UNLESS (p /\* (~* q))) Pr`--),
   REWRITE_TAC [STABLE] THEN
   REPEAT GEN_TAC THEN DISCH_TAC THEN
   ASSUME_TAC (UNDISCH_ALL (SPECL
   ...
   ASSUME_TAC (UNDISCH_ALL (SPECL 
     [(--`(p:'a->bool) /\* q`--),(--`False:'a->bool`--),
      ...
      (--`Pr:('a->'a)list`--)]
      UNLESS_thm5)) THEN
   UNDISCH_TAC (--`((((p:'a->bool) /\* q) \/* (p /\* (~* q))) UNLESS
                 ((((~*(p /\* q)) /\* (p /\* (~* q))) \/*
                   ((~*(p /\* (~* q))) /\* False)) \/*
                    (False /\* (p /\* (~* q)))))Pr`--) THEN
...
val UNLESS_cor15_lem1 = TAC_PROOF
  (([],
   (--`!p q. p /\ (~p \/ ~q) = p /\ ~q`--)),
   REPEAT GEN_TAC THEN 
   EQ_TAC THEN
...
val UNLESS_cor15_lem2 = TAC_PROOF
  (([],
   (--`!p q. p \/ (p /\ q) = p`--)),
   REPEAT GEN_TAC THEN 
   EQ_TAC THEN
  ...
  ("UNLESS_cor15",
   ...
   LIST_INDUCT_TAC THENL
     ...
     ,
      ...
      REPEAT STRIP_TAC THENL
        [
         UNDISCH_TAC (--`~(?x:num. Q x (s:'a))`--) THEN
         REWRITE_TAC [NOT_EXISTS_CONV (--`~(?x:num. Q x (s:'a))`--)] THEN
         DISCH_TAC THEN
         DISJ1_TAC THEN
         GEN_TAC THEN
         ASSUME_TAC (SPEC_ALL (ASSUME (--`!x:num. P x (s:'a)`--))) THEN
         ASSUME_TAC (SPEC_ALL (ASSUME (--`!x:num. ~Q x (s:'a)`--))) THEN
         ASSUME_TAC (UNDISCH_ALL (SPECL
           [(--`(P:num->('a->bool)) x s`--),(--`~(Q:num->('a->bool)) x s`--)]
            AND_INTRO_THM)) THEN
         ASSUME_TAC (UNDISCH (SPEC (--`s:'a`--)
             (CONJUNCT1 (SPEC (--`x:num`--) (ASSUME
           (--`!i:num. (!s:'a. P i s /\ ~Q i s ==>
                         P i(st s) \/ P i(st s) /\ Q i(st s)) /\
                         ((P i) UNLESS (\s. P i s /\ Q i s))Pr`--)))))) THEN
         UNDISCH_TAC
           (--`(P:num->('a->bool)) x ((st:'a->'a) s) \/
                P x(st s) /\ Q x(st s)`--) THEN
  ...
  ("UNLESS_cor18",
   ...
   LIST_INDUCT_TAC THENL
     ...
     ,
      ...
      REPEAT STRIP_TAC THENL
        [
         STRIP_ASSUME_TAC (SPEC (--`s:'a`--) (MP (SPEC_ALL UNLESS_STMT_thm5)
          (GEN (--`m:num`--) (CONJUNCT1 (SPEC (--`m:num`--) (ASSUME
            (--`!m:num. (!s:'a. ((P m) UNLESS_STMT q) st s) /\
                            ((P m) UNLESS q) Pr`--)))))))
        ,
         STRIP_ASSUME_TAC (REWRITE_RULE [?*] (MP
          (ASSUME (--`(!m. (((P:num->'a->bool) m) UNLESS q)Pr) ==>
                      (($?* P) UNLESS q)Pr`--))
