(* ===================================================================== *)
(* FILE		: functions.sml	(formerly start_groups.ml)		*)
...
fun EXTRAGENERALFUNCTIONS_ERR{function,message} =
    HOL_ERR{origin_structure = "ExtraGeneralFunctions",
...
fun find_match {pattern, term} =
	...
	fun find_match_aux term =
		...
		handle HOL_ERR _ =>
			...
			handle HOL_ERR _ =>
			    raise EXTRAGENERALFUNCTIONS_ERR
                                    {function = "find_match",
				     message = "no match"}
    in
	find_match_aux term
...
fun mapshape {partition = [],functions = [], unionlist = []} = []
  ...
  | mapshape _ = raise EXTRAGENERALFUNCTIONS_ERR{function = "mapshape",
                                                 message = "bad fit"}
		    ...
		    MP
		      (BETA_RULE
		       (SUBST[{thm=NOT_DEF, var=neg_var}]
			      ...
			      negated_theorem))
		      theorem
	    else raise EXTRAGENERALFUNCTIONS_ERR
	...
	if type_of new_claim = bool
		...
		([(new_claim::(fst current_goal),snd current_goal),
		 ...
		 fn [goalthm,claimthm] =>
		  ...
		  | _ => raise EXTRAGENERALFUNCTIONS_ERR
...
local
    fun tryfind f [] = 
	  raise EXTRAGENERALFUNCTIONS_ERR{function = "REDUCE_TAC-tryfind",
...
end (* structure ExtraGeneralFunctions *)
