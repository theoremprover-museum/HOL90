...
open LiteLib Type Term Dsyntax Thm Drule Conv Parse Tactic Tactical 
     Thm_cont Taut_thms Equal AC Ho_rewrite;
open Theorems Ho_resolve;
open Theory Psyntax;
...
val _ = say "Adding canonicalization tools...";
...
val (args,ONEWAY_SKOLEM_CONV) =
  ...
  in (args,fn gvs => fn tm =>
	...
	let val nfvs = intersect fvs gvs
	    val eps = mk_const("@",(type_of v --> bool_ty) --> type_of v)
	    ...
	    val th3 = PINST  [aty |-> type_of v] [P |-> atm, 
						  z |-> lhs(concl th2)] pth2
	in 
  ...
  let val PRENEX1_QCONV =
	  ...
	  in let val cname = name_of_const lop 
		 ...
		 AP_TERM lop (ABS_CONV PRENEX_QCONV r)
		    ...
		    (RAND_CONV PRENEX_QCONV THENQC PRENEX2_QCONV) tm
...
val SKELIM =
	...
	let val vars = map lhand skols
		...
		let val ((skol,v,var),rest) = 
		    remove (fn (skol,v,vars) => null (subtract vars sofar)) l
		in skol::take (v::sofar) rest
		end handle HOL_ERR _ => []
...
in
fun CNF_THEN_REFUTE cnfrefuter tm =
	cnfrefuter (split_thms [ASSUME tm] [])
...
val REFUTE =
  let val pth = TAUT (--`(~p ==> F) ==> p`--)
      ...
      handle HOL_ERR _ => refuter tm (* (trace_tm (3,"Attempting next disjunction -- ",tm); refuter tm)  *)
...
val FOL_CONV =
	...
	fun FOL1_CONV v tm =
		...
		let val snums = sort (curry op <=) nums
		    ...
		    val (atys,_) = splitlist dest_fun_ty (type_of v)
		    val otys = fst(chop_list(last tnums) atys)
