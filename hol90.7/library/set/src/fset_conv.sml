...
fun FSET_CONV_ERR{function,message} = HOL_ERR{origin_structure = "Fset_conv",
...
local
val finE = theorem "set" "FINITE_EMPTY" 
val finI = let val th1 =  theorem "set" "FINITE_INSERT" 
...
fun check st = assert (fn c => #Name(dest_const c) = st)
val bv = genvar (==`:bool`==)
val Edel = theorem "set" "EMPTY_DELETE"
val Dins = GENL [--`y:'a`--, --`x:'a`--]
                (SPECL [--`x:'a`--,--`y:'a`--] 
                       (theorem "set" "DELETE_INSERT"))
...
fun mk_comb3 (tm1,tm2,tm3) = mk_comb{Rator=mk_comb{Rator=tm1,Rand=tm2},
                                     Rand=tm3};
local
val InU  = theorem "set" "INSERT_UNION"
val InUE = theorem "set" "INSERT_UNION_EQ"
val Eu  = CONJUNCT1 (theorem "set" "UNION_EMPTY")
fun check st = assert (fn c => #Name(dest_const c) = st)
fun strip_set tm = 
...
fun itfn conv (ith,iith) x th = 
   ...
   handle _ => let val v = genvar (type_of S)
               ...
               in SUBST[{var=v,thm=th}]
                       (mk_eq{lhs=lhs,rhs=mk_comb{Rator=rator rhs,Rand=v}}) thm
               end
   end
in
...
local
val Ith = theorem "set" "IMAGE_INSERT"
and Eth = theorem "set" "IMAGE_EMPTY"
fun check st = assert (fn c => #Name(dest_const c) = st)
