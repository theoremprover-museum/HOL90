...
open Exception Lib liteLib ho_matchLib CoreHol;
open LiteLib Type Term Dsyntax Thm Drule Conv;
open Psyntax Equal Ho_rewrite Ho_match;
...
val _ = say "Adding AC reasoning tools...";
...
val DISTRIB_CONV =
	...
	let val DISTRIB0_QCONV = GEN_REWRITE_CONV I [lth, rth]
		...
		in if oper = lop then 
		    COMB2_QCONV (RAND_CONV DISTRIB2_QCONV) DISTRIB2_QCONV tm
		   else if oper = hop then
		       ((COMB2_QCONV (RAND_CONV DISTRIB2_QCONV)
			       DISTRIB2_QCONV)
		       THENQC DISTRIB1_QCONV) tm
			else ERR("DISTRIB2_QCONV","")
		end
	in TRY_CONV DISTRIB2_QCONV
	end
    in fn (lth0,rth0) =>
	let val (lop0,[_,htm]) = strip_comb(rand(snd(strip_forall(concl lth0))))
	    val hop0 = rator(rator htm)
	in if type_vars_in_term hop0 = []
		...
		in fn tm => 
		    ...
		    in RAW_DISTRIB_CONV
			(INST_fn lth0) (INST_fn rth0) 
			(inst_fn hop0) (inst_fn lop0) tm
		    end
		end
	    handle e => WRAP_ERR("DISTRIB_CONV",e)
	end
    end;
...
val ASSOC_CONV =
  let fun MK_ASSOC_CONV oper assoc t1 t2 t3 =
	  ...
	  in ASSOC_ACC_CONV l (ASSOC_CONV r) handle  _ => ASSOC_TAC_CONV l r
	  end
	  ...
	  and ASSOC_ACC_CONV tm ath =
	      ...
	      in TRANS (INST [t1 |-> l, t2 |-> r, 
			      t3 |-> (rand(rator(concl ath)))] assoc) rth
	      end
	  handle  _ => AP_TERM (mk_comb(oper,tm)) ath
  ...
  in fn rassoc =>
      ...
      in if type_vars(type_of oper) = [] then MK_ASSOC_CONV oper assoc t1 t2 t3 
	 else fn tm =>
	     let val xop = rator(rator tm)
		 val ([],tyin) = match_term [] oper xop
		 val inst_fn = inst tyin
		 val assoc' = INST_TYPE tyin assoc
		 and t1' = inst_fn t1 and t2' = inst_fn t2 and t3' = inst_fn t3
	     in  MK_ASSOC_CONV xop assoc' t1' t2' t3' tm
	     end
  	     handle  _ => REFL tm
      end
  end
