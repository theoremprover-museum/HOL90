 ...
 *****************************************************************************)
local
fun mk_rsubst2 L = Lib.map2 (fn r1 => fn r2 => {redex = r2, residue = r1}) L
fun variants (_,[]) = []
  | variants (av, h::rst) =
...
(* Fix from Karsten Nyblad at tf.dk *)
fun SUB_CONV conv tm =
   if (is_comb tm)
   then let val {Rator,Rand} = dest_comb tm
        in MK_COMB(conv Rator, conv Rand)
...
fun ORELSEQC conv1 conv2 (tm:term) =
 let val (th,real_thm) = (conv1 tm,true) 
                         handle UNCHANGED => (TRUTH,false) 
                                      | _ => (conv2 tm,true)
 in if real_thm 
    then th 
...
fun CHANGED_QCONV conv (tm:term) =
   let val th = conv tm handle UNCHANGED => raise CHANGED_QCONV_ERR
       val {lhs,rhs} = dest_eq (concl th)
   in
   if (aconv lhs rhs)
...
fn tm =>
  ...
  if (free_in v tm) 
  then raise CONV_ERR{function = "X_SKOLEM_CONV",
             message = ("`"^(#Name(dest_var v))^"` free in the input term")}
...
fun FUN_EQ_CONV tm = 
   let val vars = free_vars tm 
       ...
       val varnm = if (Type.is_vartype ty1) 
                   then "x"
                   else Portable.String.str
		        (hd(Portable.String.explode
...
fun X_FUN_EQ_CONV x tm = 
   ...
   then raise CONV_ERR{function = "X_FUN_EQ_CONV",
        ...
        then raise CONV_ERR{function = "X_FUN_EQ_CONV",
                   message = (#Name(dest_var x) ^" is a free variable")}
        else let val l = lhs tm handle _ => 
                 ...
                 val ty1 = case (Type.dest_type (type_of l))
                             of {Tyop = "fun", Args = [ty1,_]} => ty1
                              | _ => raise CONV_ERR{function = "X_FUN_EQ_CONV",
                                         message = "lhs and rhs not functions"}
             in
             if (not (ty1 = type_of x))
             then raise CONV_ERR{function = "X_FUN_EQ_CONV",
                        message = (#Name(dest_var x) ^ " has the wrong type")}
