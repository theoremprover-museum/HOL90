...
let val EVAL = (--`EVAL : comm -> ^state -> ^state -> bool`--) 
in
new_inductive_definition{name="trans", fixity=Prefix,
   patt=(--`^EVAL C s1 s2`--, []),
   rules=[
           { (* ----------------------------------------------------- *)
conclusion=                   (--`^EVAL skip s s`--)                  ,
hypotheses=[],
side_conditions=[]},
...
{hypotheses=[(--`^EVAL C1 s1 s2`--),(--`^EVAL C2 s2 s3`--)],
           (* ---------------------------------------------*)
conclusion=           (--`^EVAL (C1;;C2) s1 s3`--)          ,
side_conditions=[]},


{hypotheses=[--`^EVAL C1 s1 s2`--], side_conditions=[--`(B:^bexp) s1`--],
            (* ----------------------------------------------------- *) 
conclusion=            (--`^EVAL (if B C1 C2) s1 s2`--)             },


{hypotheses=[--`^EVAL C2 s1 s2`--],side_conditions=[--`~((B:^bexp) s1)`--],
            (*--------------------------------------------------------- *) 
conclusion=               (--`^EVAL (if B C1 C2) s1 s2`--)            },

{hypotheses=[],side_conditions=[--`~((B:^bexp) s)`--],
         (* ------------------------------------------------- *) 
conclusion=          (--`^EVAL (while B C) s s`--)             },


{hypotheses=[(--`^EVAL C s1 s2`--),(--`^EVAL (while B C) s2 s3`--)],
 side_conditions=[--`(B:^bexp) s1`--],
       (* ------------------------------------------------- *)
conclusion=        (--`^EVAL (while B C) s1 s3`--)             }]}
end;
