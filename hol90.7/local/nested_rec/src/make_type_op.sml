...
functor MakeTypeOpTable (structure RecursorThms : RecursorThmsSig)
	...
	val tyop_suffix = "_" ^ String.substring
	...
	val symbolic_ords = map ord (explode "#?+*/\\=<>&%@!,:;_|~-")
	    
	fun rename_symbolic s = "ch"^(Lib.int_to_string(ord s))
	    
	fun mk_symbolic_free_const_name n =
	    if Lib.mem (ord n) symbolic_ords
		then underscore (map rename_symbolic (explode n))
	    else n
	...
	fun process_thms rec_thm_list =
	    ...
	    in
		fold enter_type_op rec_thm_list empty_table
	    end
