...
(* ML pattern matching makes exp 0 0 be 0. *)
fun exp 0 m = 0
  | exp n 0 = 1
  | exp n m = n * exp n (m-1);
...
fun it_min alist = 
   itlist (fn (x:int) => fn y => if (x <= y) then x else y) alist (hd alist);
...
fun iota bot top = 
   let fun iot i = if (i > top) then [] else i::(iot (i+1))
   in iot bot
   end;
 ...
 *---------------------------------------------------------------------------*)
local fun SUBSETS_ERR s = raise AC_LIB_ERR{function="subsets",message=s}
      val MAX_BASIS_SIZE = 29 (* powers of 2 from 0 to MAX_BASIS_SIZE *)
      val powers_of_two = Vector.fromList(map (Word.fromInt o (exp 2)) 
                                              (iota 0 MAX_BASIS_SIZE))
in
fun subsets p (s:'a list) =
   let val svect = Vector.fromList s
       val sze = Vector.length svect
       val two_exp_sze = exp 2 sze handle Overflow => SUBSETS_ERR"too many"
       fun word_to_subset w =
          let fun is_bit_set i = 
                 (Word.toInt(Word.andb(w, Vector.sub(powers_of_two,i))) <> 0)
              fun loop i = 
                if (i>=sze) then []
        ...
        fun loop i =
           if (i>=two_exp_sze) then []
           else let val sset = word_to_subset (Word.fromInt i)
                in if (p sset) then sset::loop(i+1) else loop(i+1)
                end
   in loop 0
   end
end;

fun powerset l = subsets (fn _ => true) l;
...
fun op_lex_gt eq_func order =
   let fun gt [] [] = false
         | gt (t1::rst1) (t2::rst2) =
              ...
              else if (eq_func t1 t2) then gt rst1 rst2
                   else false
 ...
 * Should generalize to other data types, as with sorting.
 ---------------------------------------------------------------------------*)
fun largest_el alist = itlist (curry Portable.Int.max) alist 0;
...
(*---------------------------------------------------------------------------
 * Component-wise greater-than-or-equal-to (big conjunction) a
