...
structure Cond_rewrite : Cond_rewrite_sig =
struct
...
open Term Dsyntax Match Thm Drule Conv Tactic Tactical Thm_cont Rewrite;
infix ## THEN THENL;
infix ORELSEC;
...
fun match_ok vs (l:(term subst * hol_type subst) list) =
  ((length l) = 1) andalso
   let val subst = hd l
...
fun MATCH_SUBS1 th fvs asm (m1:(term subst * hol_type subst)) =
...
(*----------------------------------------------------------------------
 * Taken out by DRS as top level opens are not allowed by CM - now
 * in load.sml
 *--------------------------------------------------------------------*)
