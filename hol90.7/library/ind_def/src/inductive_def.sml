...
local
fun itfn tm ty = mk_type{Tyop="fun",Args=[(type_of tm),ty]}
val bool = ==`:bool`==
in
fun mk_predv ts = mk_var{Name="P",Ty=itlist itfn ts bool}
end;
...
fun make_definition (pat,ps) rules =
   let val rules = map (fn {hypotheses,side_conditions,conclusion} =>
                       ...
                       rules
       val vs = Compat.freesl (flatten (map (fn (x,y) => (y::x)) rules)) 
...
local
val ERR = IND_DEF_ERR{function="",message=""}
in
...
local
fun efn v th =  EXISTS(mk_exists{Bvar=v,Body=concl th},v) th
in
...
fun crul rel th =
   ...
   then let val th1 = CONV_RULE LIST_BETA_CONV th 
        in CONJUNCT1 (CONV_RULE (REWR_CONV NOT_IMP) th1) 
        end
   else th;
...
fun derive_cases_thm (rules,ind) =
   let val (vs,{ant,conseq}) = (I ## dest_imp) (strip_forall (concl ind))
       ...
       val fthm = NOT_INTRO (DISCH (subst [{redex=P,residue=pred}] ant)
                                          (MP thm3 contr))
       fun sfn eqs = SUBST(map2 (fn th => fn v => {var=v, thm=th})
