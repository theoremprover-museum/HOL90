...
open Type Term Dsyntax Thm Theory Drule Conv;
infix ##;
...
local
exception FAIL
val finE = theorem "pred_set" "FINITE_EMPTY"
val finI = 
   let val th1 =  theorem "pred_set" "FINITE_INSERT"
...
local
val inI = theorem "pred_set" "IN_INSERT"
val inE = GEN (--`x:'a`--) 
              (EQF_INTRO (SPEC (--`x:'a`--) 
                               (theorem "pred_set" "NOT_IN_EMPTY")))
val T = (--`T`--) 
and F = (--`F`--) 
and gv = genvar boolty
val DISJ = AP_TERM (--`\/`--)
...
val Edel = theorem "pred_set" "EMPTY_DELETE" 
val Dins = GENL [(--`y:'a`--),(--`x:'a`--)] 
                (SPECL [(--`x:'a`--),(--`y:'a`--)]
                       (theorem "pred_set" "DELETE_INSERT")) 
fun del_conv conv (eth,ith) x S = 
   let val (_,[y,S']) = (check "INSERT" ## I) (strip_comb S) 
       ...
       val thm2 = SUBST [{var=v,thm=rthm},{var=bv,thm=eql}] pat thm
...
local
val InU  = theorem "pred_set" "INSERT_UNION"
val InUE = theorem "pred_set" "INSERT_UNION_EQ"
val Eu  = CONJUNCT1 (theorem "pred_set" "UNION_EMPTY")
fun strip_set tm = 
...
fun mkIN x s =
   let val ty = type_of x 
       ...
       val sty = fun_ty ty boolty
       val INty = fun_ty ty (fun_ty sty boolty)
...
fun itfn conv (ith,iith) x th = 
   let val (_,[S,T]) = strip_comb(lhs(concl th))
       ...
       val thm2 = SUBST [{var=v,thm=th},{var=bv,thm=eql}] pat thm 
   in
   TRANS thm2 (COND_CONV (rand(concl thm2)))
   handle _ => let val v = genvar (type_of S)
                   val thm = SPEC T (SPEC S (SPEC x iith))
                   val {lhs,rhs} = dest_eq(concl thm)
                   val r = rator rhs
               in SUBST [{var=v,thm=th}] 
                        (mk_eq{lhs=lhs, rhs = mk_comb{Rator=r,Rand=v}}) thm 
               end
   end
in
fun UNION_CONV conv tm =
   let val (_,[S1,S2]) = (check "UNION" ## I) (strip_comb tm)
       ...
       val ith = INST_TYPE [{redex=alpha_ty,residue=ty}] InU
       val iith = INST_TYPE [{redex=alpha_ty,residue=ty}] InUE
   in
...
local
val absth = 
   let val th = theorem "pred_set" "ABSORPTION"
       val th1 = fst(EQ_IMP_RULE(SPECL[(--`x:'a`--),(--`s:'a->bool`--)] th))
   in GENL [(--`x:'a`--),(--`s:'a->bool`--)] th1
   end
fun mkIN x s =
   let val ty = type_of x 
       val sty = mk_type{Tyop="fun",Args=[ty,boolty]}
       val INty = mk_type{Tyop="fun",
                          Args=[ty,mk_type{Tyop="fun",Args=[sty,boolty]}]}
...
local
val Ith = theorem "pred_set" "IMAGE_INSERT" 
and Eth = theorem "pred_set" "IMAGE_EMPTY"
