...
val set = ty_antiq(==`:'a->bool`==);

Library.load_library_in_place num_lib;
Add_to_sml.add_theory_to_sml "num";
Add_to_sml.add_theory_to_sml "prim_rec";
Add_to_sml.add_theory_to_sml "arithmetic";

val path_prefix = (!Globals.HOLdir)^"library/pred_set/"
fun use_lib_code s =
   let val p = path_prefix^"src/"^s
   in  compile (p^".sig"); 
       compile (p^".sml")
   end;

val thy_path = path_prefix^"theories/"^Globals.theory_file_type^"/"
val _ = Lib.clean_directory thy_path;
val _ = theory_path := thy_path::(!theory_path);
...
val SPECIFICATION = new_infix_definition("SPECIFICATION", 
   (--`!P x. $IN (x:'a) (P:^set) = P x`--), 450);
...
val lemma =
    ...
    (([], (--`!s x. x IN s ==>  !f:'a->'b. (f x) IN {f x | x IN s}`--)),
     REPEAT STRIP_TAC THEN CONV_TAC Gspec.SET_SPEC_CONV THEN
...
val SET_MINIMUM = 
    ...
    ("SET_MINIMUM",
     ...
     [IMP_RES_THEN (ASSUME_TAC o ISPEC (--`M:'a->num`--)) lemma THEN
      let val th = Gspec.SET_SPEC_CONV (--`(n:num) IN {M x | (x:'a) IN s}`--) 
...
val PSUBSET_DEF =  new_infix_definition("PSUBSET_DEF", 
...
val IN_UNION = store_thm
     ("IN_UNION",
      ...
      CONV_TAC (ONCE_DEPTH_CONV Gspec.SET_SPEC_CONV) THEN
...
val INSERT_DEF = 
    ...
    ("INSERT_DEF", (--`INSERT (x:'a) s = {y | (y = x) \/ y IN s}`--),500);
...
(* --------------------------------------------------------------------- *)
(* A theorem from homeier@orfn g =>aero.uniblab (Peter Homeier)		*)
(* --------------------------------------------------------------------- *)
val CARD_DIFF =
    store_thm
    ("CARD_DIFF",
...
val IMAGE_11_INFINITE = 
    ...
    ("IMAGE_11_INFINITE",
     ...
     let val alpha = ==`:'a`==
         and beta = ==`:'b`==
         val theta = [{redex=beta,residue=alpha},
                      {redex=alpha,residue=beta}]
         val thm = INST_TYPE theta IMAGE_FINITE 
...
val g_cases = 
    ...
    ((gdef, (--`(!s. FINITE s ==> ?x:'a. ~(x IN s)) ==>
     ...
     DISCH_THEN (STRIP_THM_THEN MP_TAC o 
                 CONV_RULE Num_lib.EXISTS_LEAST_CONV) THEN
  ...
  [PURE_ONCE_REWRITE_TAC [EXTENSION] THEN
   ...
   [REPEAT GEN_TAC THEN
    ...
    [REPEAT STRIP_TAC THEN REFL_TAC,
     ...
     in
     FILTER_ASM_REWRITE_TAC is_less [LESS_REFL] 
     end THEN
    ...
    [EXISTS_TAC (--`CARD (s:^set)`--) THEN
     ...
     FILTER_ASM_REWRITE_TAC (fn t => (not(lhs t = (--`s:^set`--)))
                                     handle _ => true) [],
     SUBST1_TAC (ASSUME (--`x:'a = (n < CARD (s:^set) => f n | e)`--)) THEN
